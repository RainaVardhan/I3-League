...
**Any white/paper button needs this same hard shadow.** A paper-background button that only gets the plain ghost's faint `0 2px 0 rgba(16,33,61,.06)` resting shadow reads as flat and disconnected from the rest of the button system. Whenever a button sits on a paper (or otherwise light) fill and needs to read as a real, pressable button rather than a plain outline, give it the `ghostStrong` treatment: `0 5px 0 var(--dim)` resting shadow, same vertical-only lift/press behavior as primary. This applies whether or not it's paired with a primary button next to it.
...
2. **Page bar (navy).** A full-width ink band between the hero and the pages, the Pattern D bridge used as navigation: ink fill, shadow `0 14px 30px rgba(16,33,61,.14)`, inset `var(--gutter)`, row spanning the full page width. It lists every page of the stage as **plain tabs, not boxed buttons** (a row of nine to nineteen white buttons read as competing calls to action). Nothing is drawn around a tab: no dividers, no fills, no borders. A tab is 56px tall, `0 15px`, with a 13.5px 600 Hanken Grotesk label in `#c6d0df` (the canonical body-on-ink grey, Section 21.8) and a 10px 650 mono number (`01`, `.1em`, `rgba(187,200,233,.55)`, the ink-bridge mono label held back). Labels are sentence case, not mono uppercase: a row of that many mono labels is wider than a laptop screen. A single `rgba(255,255,255,.11)` hairline runs across the foot of the band, and **the open page is paper-white text with a 2px `--blue-soft` underline sitting on that rule** plus a `#b8c8f8` number (blue-soft is a plane color here and is never text). Hover lifts the label to `--paper` and the number to `#b8c8f8`; the underline is drawn on every tab at zero opacity, so nothing moves as the open page changes. Focus is a `--blue-soft` ring, since the global cobalt one is invisible on ink. The list is pulled out by one tab's side padding so the first label lines up with the gutter. The tabs sit on one line and the row scrolls sideways (scrollbar hidden) when it is too long, never wraps. Each shows the **short page name** (the same label the sidebar uses). A locked page (Review, before submit on Investigate) is dimmed with a lock icon and not clickable. Choosing a page scrolls the bar to just under the topbar so the bar and the top of the page stay in view. The open page is kept in the URL (`?page=empathy`, updated without adding history entries), so a reload or a shared link stays on the same page; an unknown or locked page falls back to the first page (or Review once submitted).
...
**Sidebar section list.** The stage's pages are listed in the app shell's left sidebar, under the open stage's row, as **one list shown two ways, on one connecting line in both**: dots on a thread, centred on the stage numbers' own axis in the rail and flush with the slot's own left edge in the open flyout, with the number and page name to its right. The dot, its ink ring, its four states (behind / open / hover / locked) and the thread's fixed 10px end-fade are **identical CSS in both states** (nothing in `.shellOpen` overrides their size or color) — only the thread's `left` position and the row's own padding change, since the flyout's row isn't centred the way the rail's is. This was a second attempt: an earlier version drew a second, separate `border-left` indent guide next to the new thread, which read as two parallel lines; that guide is gone, so the thread is the only line. A page **behind** the open one is a solid `#a9bdf2` dot lit by a small cobalt glow; a page **ahead** is a solid, hand-flattened `#3b4d70` (28% `#a9bdf2` over `--ink`, opaque on purpose — a translucent fill always lets the thread show through, which is what caused the bug this replaced); **locked** Review is a fine hollow ring on an opaque `--ink` fill (not `transparent`, for the same reason) and inert. Hover grows a dot with a soft overshoot easing and a brighter glow. **The open page is a coral dot** with a slow 2.6s breathing glow (`subBeadBreathe`, dropped under `prefers-reduced-motion`), the same coral the `CURRENT` label uses on the stage row above. Each dot keeps its page name as the button's accessible name and native tooltip. The rail carries no per-page numerals and no counter; the open flyout shows the real zero-padded number and name beside each dot.

**Clicking a stage shows its sections; the row itself never navigates.** Every unlocked row (active or not) is a wrapper (the row's usual look) around a Link whose own click is always prevented, plus a separate arrow button — both bubble into the wrapper's click handler, so the whole row is the toggle target, not just the arrow. The only real navigation left in "Your journey" is a click on one of the *pages* inside an expanded list (or on Dashboard). **Every unlocked stage can expand, not only the one on screen.** The active stage's list is the live one (StageSections, portaled in); every other unlocked stage's list is a plain, static preview built from the same page configs (`src/lib/stage-sidebar-pages.ts`), with real links straight to that stage's pages, on the same dot-and-thread row styling — just with every dot in the neutral "not current" color, since the app has never tracked which page you left off on within a stage you aren't viewing. A locked stage can expand too: its row is a `<span>`, not a `<Link>` (CLAUDE.md "Sequential stage unlocking"), and its preview list's rows are `<span>`s as well, each with the same lock icon and hollow-ring dot Review uses before it's unlockable, and none of them navigate. Each stage's own expand/collapse state is independent (`Partial<Record<StageName, boolean>>`); the active stage starts open (matching what the sidebar always showed), every other stage starts closed.
