// Browser-safe half of src/lib/journal.ts (constants, types, pure helpers, no
// database code). Client components import from here; journal.ts re-exports it.
...
// Earliest date a journal entry may carry: the season's open date, or a couple
// of days ago if the season has not opened yet. Without that second half, the
// date field's default (today) sits before the minimum and nobody could write
// an entry until the season opened. Entry dates are date-only (midnight UTC),
// and a student's "today" can be a day behind UTC, so the fallback is UTC
// midnight two days back rather than "now".
export function journalEarliestDate(seasonOpenDate: Date, now: Date = new Date()): Date {
  if (seasonOpenDate.getTime() <= now.getTime()) return seasonOpenDate;
